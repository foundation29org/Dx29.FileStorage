...
namespace Dx29.Services
{
    public partial class BlobStorage
    {
        ...
        public async Task MoveBlobAsync(string containerSource, string containerTarget, string source, string target)
        {
            await CopyBlobAsync(containerSource, containerTarget, source, target, true);
        }
        ...
        public async Task CopyBlobAsync(string containerSource, string containerTarget, string source, string target, bool deleteSource = false)
        {
            var clientSource = BlobServiceClient.GetBlobContainerClient(containerSource);
            var clientTarget = BlobServiceClient.GetBlobContainerClient(containerTarget);
            var sourceClient = clientSource.GetBlockBlobClient(source);
            var targetClient = clientTarget.GetBlockBlobClient(target);
            await targetClient.StartCopyFromUriAsync(sourceClient.Uri);
            if (deleteSource)
            {
                await sourceClient.DeleteAsync();
